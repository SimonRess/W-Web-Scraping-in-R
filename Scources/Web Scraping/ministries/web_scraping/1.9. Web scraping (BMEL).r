  ...
  #extract macro information (Name/Link/Date) about legislative proposals
    extractLinksNamesDates <- function(page.nr = nr.pages) {
      ...
      #extract number of pages
        nr.pages <- html_nodes(page, xpath = "/html/body/div/div/div/div[3]/div/div[3]/div/nav/ul/li[5]/a/text()") %>%
          html_text() %>%
          as.numeric()
      ...
      #extract infos of all pages
        for(i in 1:nr.pages) {
          ...
          #list of links
            links <- html_nodes(page, css = "#content > div:nth-child(3) > div > div.searchresult > div > div > div > h2 > a") %>%
              html_attr("href") %>%
              paste0("https://www.bmel.de/",.) %>%
              strsplit(.,";") %>% #cut identification id in url, e.g.: "https://www.bmas.de/DE/Service/Gesetze/arbeitsschutzkontrollgesetz.html;jsessionid=C26FA76B63B062996AB37CEF6C159182"  
              unlist() %>%
              grep("https://", ., value=TRUE) # needed to delete "jsessionid=C26FA76B63B062996AB37CEF6C159182" part of the url
              
          
          #list of names
            names <- html_nodes(page, css = "#content > div:nth-child(3) > div > div.searchresult > div > div > div > h2 > a") %>%
              html_text() %>%
              trimws(which = c("both")) 
          ...
          #list of last-update-time
            last.updates <- html_nodes(page, css = "#content > div:nth-child(3) > div > div.searchresult > div > div > div > p.c-searchteaser__meta > time") %>%
              html_attr("datetime") %>%
              as.Date() #transform into international date format
          ...
          if(i<=nr.pages-1) {
            #Click next-page-button up to the penultimate page 
              search <- remDr$findElement(using = 'xpath', 
                                          paste0("/html/body/div/div/div/div[3]/div/div[3]/div/nav/ul/li/a[contains(@title, 'Seite ", i+1, "')]"))
...
#----------------------------------#  
  ...
  remDr$navigate("https://www.bmel.de/SiteGlobals/Forms/Suche/DE/Gesetzestexte/Gesetzestexte_Formular.html;jsessionid=CAC168DED2D2D3AA577736D6CC695376.internet2831")
...
#-------------------------------------------------------#         
  ...
  #Add information for all legislative processes
    ...
    #Save Link to "Referentenentwurf" / Link to "Regierungsentwurf" / Link to "Verkündetes Gesetz" / names of actor (who submitted a statement) / links to the statments
      ...
      #Save informatin within legislative process
        ...
        #1."Referentenentwurf" keep only information of the last
          #Link
            Referentenentwurf.Link <- html_node(page, xpath = "/html/body/div[1]/div/div/div[3]/div/div/div/div/div/div[./div/h2[contains(text(), 'Referentenentwurf')]]") %>%
              html_nodes(., xpath = "div/ul/li/a") %>%
              html_attr("href") %>%
              {ifelse(length(.)>0, paste0("https://www.bmel.de", .), NA)}
        ...
        #2."Regierungsentwurf" keep only information of the last
          #Link
            Regierungsentwurf.Link <- html_node(page, xpath = "/html/body/div[1]/div/div/div[3]/div/div/div/div/div/div[./div/h2[contains(text(), 'Regierungsentwurf')]]") %>%
              html_nodes(., xpath = "div/ul/li/a") %>%
              html_attr("href") %>%
              {ifelse(length(.)>0, paste0("https://www.bmel.de", .), NA)}
      ...
      #Several informations within legislative process    
        #4.  Link to "Verkündetes Gesetz"
          ...
          actors <- html_nodes(page, xpath = "/html/body/div[1]/div/div/div[3]/div/div/div/div/div/div[./div/h2[contains(text(), 'Stellungnahmen')]]") %>%
            html_nodes(., xpath = "div/ul/li/a") %>%
            html_text() %>%
            gsub("Stellungnahme ", "", .) %>%
            gsub("\\(PDF(.*)", "", .) %>% # "\\" needed before "(" to declare it as character
            gsub("[[:space:]]$", "", .) %>% # remove (non-breaking)-space-character ["&nbsp"]in string in R
            trimws(which="both")  # delete "normal" white spaces
            
              if(length(actors)==0) actors <- NA
            
          
          statement.links <- html_nodes(page, xpath = "/html/body/div[1]/div/div/div[3]/div/div/div/div/div/div[./div/h2[contains(text(), 'Stellungnahmen')]]") %>%
            html_nodes(., xpath = "div/ul/li/a") %>%
            html_attr("href") %>%
            paste0("https://www.bmel.de", .)
          
              if(statement.links=="https://www.bmel.de") statement.links <- NA
    ...
    #Add "ministry" variable
      data.full$ministry <- "BMEL"   
   
    #Save dataset
      write.csv2(data.full, "./datasets/BMEL.csv", row.names = F)
      saveRDS(data.full, "./datasets/BMEL.rds")
