  ...
  #extract macro information (Name/Link/Date) about legislative proposals
    extractLinksNamesDates <- function() {
      ...
      #extract number of pages
        nr.pages <- html_nodes(page, xpath = "/html/body/div/div/div/div[2]/div[4]/div[1]/div/div[3]/ul/li[6]/a/text()") %>%
          html_text() %>%
          as.numeric()
        ...
        for(i in 1:nr.pages) {
          ...
          #list of links
            links <- grep("https://", # needed to delete "jsessionid=C26FA76B63B062996AB37CEF6C159182" part of the url
                      unlist(
                      strsplit(
                        paste0("https://www.bmjv.de/",
                          html_attr(
                            html_nodes(page, xpath = "/html/body/div/div/div/div[2]/div[4]/div[1]/div/div[2]/div/div/div/h3/a")
                          ,"href")
                        )
          ...
          #list of names
              names <- html_nodes(page, xpath = "/html/body/div/div/div[2]/div[2]/div[4]/div[1]/div/div[2]/div/div/div/h3/a") %>%
                html_text() %>%
                as.character() %>%
                str_extract('(?<=verfahren)(.*)')
                
                
                #html_text(html_nodes(page, xpath = "/html/body/div/div/div/div[2]/div[4]/div[1]/div/div[2]/div/div/div/h3/a/text()"))[c(F,T)] %>% # two elements per name -> first "\n" -> keep only the second which contains the name
                #trimws(which = c("both")) %>%
          ...
          #list of last-update-time
            last.updates <- html_text(html_nodes(page, xpath = "/html/body/div/div/div[2]/div[2]/div[4]/div[1]/div/div[2]/div/div/div/h3/a/span[1]/text()[1]")) %>%
          ...
          if(i<=nr.pages-1) {
            #Click next-page-button up to the penultimate page 
              next.page <- remDr$findElement(using = 'xpath', 
                                          "/html/body/div/div/div/div[2]/div[4]/div[1]/div/div[3]/ul/li/a[contains(@class, 'forward button ')]") # Information about the "Suchen" button
...
remDr$navigate("https://www.bmjv.de/SiteGlobals/Forms/Suche/Gesetzgebungsverfahrensuche_Formular.html")
...
#---------------------------------------------------------------------------------#         
  ...
  #Add information for all legislative processes
    ...
    #Save Link to "Referentenentwurf" / Link to "Regierungsentwurf" / Link to "Verkündetes Gesetz" / names of actor (who submitted a statement) / links to the statments
      ...
      #Save informatin within legislative process
        ...
        #Determine acticity type  
          Vorgangstyp <- html_nodes(page, xpath = "/html/body/div/div/div[2]/div[2]/div[4]/div[1]/h1/span[1]/text()[2]") %>%
            html_text() %>%
            ifelse(str_detect(., "esetz") == TRUE, "Gesetz", .) %>%
            ifelse(str_detect(., "erordnung") == TRUE, "Verordnung", .)

          
          Vorgangstyp <- ifelse(length(Vorgangstyp)<=1 & str_detect(data$names[i], "esetz") == TRUE, "Gesetz", Vorgangstyp) #delete G, because of capital letter
          Vorgangstyp <- ifelse(length(Vorgangstyp)<=1 & str_detect(data$names[i],"erordnung") == TRUE, "Verordnung", Vorgangstyp) #delete V, because of capital letter
          
          
          
      
        #1."Referentenentwurf" keep only information of the last
          #Link (sometimes several documents  [because its a cluster of legislative processes] -> take only first one [html_node instead of html_nodes])
            Referentenentwurf.Link <- html_node(page, xpath = "/html/body/div/div/div/div/div/div/div/div[h2[starts-with(text(), 'Referent')]]/div/ul/li/a") %>% # get url when headline is "Referent" because it's sometimes "Referentenentwurf" or "Referenten-Entwurf"
              html_attr("href")
            
              if(length(Referentenentwurf.Link)!=0 & !is.na(Referentenentwurf.Link)) {
                Referentenentwurf.Link <- paste0("https://www.bmjv.de/",Referentenentwurf.Link)
              } else{
                Referentenentwurf.Link <- as.character(NA) 
                }
            
          #Date (sometimes several documents  [because its a cluster of legislative processes] -> take only first one [html_node instead of html_nodes])
            Referentenentwurf.Date <- html_node(page, xpath = "/html/body/div/div/div/div/div/div/div/div[h2[starts-with(text(), 'Referent')]]/div/ul/li/span") %>% # get url when headline is "Referent" because it's sometimes "Referentenentwurf" or "Referenten-Entwurf"
              html_text() %>%
              str_remove(.,"Datum") %>%
              as.Date(., format='%d. %B %Y') # not available
 
                if(length(Referentenentwurf.Date)==0) Referentenentwurf.Date <- NA #If there is no date -> NA                               
            
            
        #2."Regierungsentwurf" keep only information of the last
            #Link (sometimes several documents  [because its a cluster of legislative processes] -> take only first one [html_node instead of html_nodes])
              Regierungsentwurf.Link <- html_node(page, xpath = "/html/body/div/div/div/div/div/div/div/div[h2[starts-with(text(), 'Regierung')]]/div/ul/li/a") %>% # get url when headline is "Referent" because it's sometimes "Referentenentwurf" or "Referenten-Entwurf"
                html_attr("href")
              
              if(length(Regierungsentwurf.Link)!=0 & !is.na(Regierungsentwurf.Link)) {
                Regierungsentwurf.Link <- paste0("https://www.bmjv.de/",Regierungsentwurf.Link)
              } else{
                Regierungsentwurf.Link <- as.character(NA) 
              }
            
            #Date (sometimes several documents  [because its a cluster of legislative processes] -> take only first one [html_node instead of html_nodes])
              Regierungsentwurf.Date <- html_node(page, xpath = "/html/body/div/div/div/div/div/div/div/div[h2[starts-with(text(), 'Regierung')]]/div/ul/li/span") %>% # get url when headline is "Referent" because it's sometimes "Referentenentwurf" or "Referenten-Entwurf"
                html_text() %>%
                str_remove(.,"Datum") %>%
                as.Date(., format='%d. %B %Y') # not available
                
                  if(length(Regierungsentwurf.Date)==0) Regierungsentwurf.Date <- as.Date(NA) #If there is no date -> NA
              
              
        #3."Gesetz"
          #Link (sometimes several documents  [because its a cluster of legislative processes] -> take only first one [html_node instead of html_nodes])
              Gesetz.Link <- html_node(page, xpath = "/html/body/div/div/div/div/div/div/div/div[h2[starts-with(text(), 'Bundesgesetzblatt')]]/div/ul/li/a") %>% # get url when headline is "Referent" because it's sometimes "Referentenentwurf" or "Referenten-Entwurf"
              html_attr("href")
            
            if(length(Gesetz.Link)!=0 & !is.na(Gesetz.Link)) {
              Gesetz.Link <- paste0("https://www.bmjv.de/",Gesetz.Link)
            } else{
              Gesetz.Link <- as.character(NA) 
            }
            
          #Date (sometimes several documents  [because its a cluster of legislative processes] -> take only first one [html_node instead of html_nodes])
            Gesetz.Date <- html_node(page, xpath = "/html/body/div/div/div/div/div/div/div/div[h2[starts-with(text(), 'Bundesgesetzblatt')]]/div/ul/li/span") %>% # get url when headline is "Referent" because it's sometimes "Referentenentwurf" or "Referenten-Entwurf"
              html_text() %>%
              str_remove(.,"Datum") %>%
              as.Date(., format='%d. %B %Y') # not available
            
                if(length(Gesetz.Date)==0) Gesetz.Date <- as.Date(NA) #If there is no date -> NA
          
                
      #Several informations within legislative process    
        #4. actor name                        
          actors <- html_nodes(page, xpath = "/html/body/div/div/div/div/div/div/div/div[h2[contains(text(), 'Stellungnahme')]]/div/ul/li/a/text()") %>% # get url when headline is "stellung" because it's  sometimes "Stellungnahme" or "Verbändestellungnahmen"
            html_text() %>%
        ...
        #5. Date of the statement
          statement.date <- html_nodes(page, xpath = "/html/body/div/div/div/div/div/div/div/div[h2[contains(text(), 'Stellungnahme')]]/div/ul/li/span") %>% # get url when headline is "Referent" because it's sometimes "Referentenentwurf" or "Referenten-Entwurf"
            html_text() %>%
            str_remove(.,"Datum") %>%
            as.Date(., format='%d. %B %Y')
          
              if(length(statement.date)==0) statement.date <- as.Date(NA) #If there is no date -> NA
            
        #6. Link to statement  
          statement.links <- html_nodes(page, xpath = "/html/body/div/div/div/div/div/div/div/div[h2[contains(text(), 'Stellungnahme')]]/div/ul/li/a") %>% # get url when headline is "Referent" because it's sometimes "Referentenentwurf" or "Referenten-Entwurf"
            html_attr("href")
          
          if(length(statement.links)!=0) {
            statement.links <- paste0("https://www.bmjv.de",statement.links)
          } else{
        ...
        data.actors <- data.frame(links = append(as.character(data.actors$links), rep(data$links[i], n.actors)),
                           ...
                           Vorgangstyp = append(data.actors$Vorgangstyp, rep(Vorgangstyp, n.actors)),
                             ...
                             Gesetz.Date = append(data.actors$Gesetz.Date, rep(Gesetz.Date, n.actors)),
                               ...
                               statement.date = append(data.actors$statement.date, statement.date[1:n.actors]), # There are sometimes dates without actors&statements -> not take them (e.g.: https://www.bmjv.de/SharedDocs/Gesetzgebungsverfahren/DE/MaMoG.html)
                               statement.links = append(data.actors$statement.links, statement.links),
                           stringsAsFactors = FALSE)

      #clear working space
        rm(Vorgangstyp, Referentenentwurf.Link, Referentenentwurf.Date, Regierungsentwurf.Link, Regierungsentwurf.Date,
    ...
    #Add "ministry" variable
      data.full$ministry <- "BMJV"  
   
    #Save dataset
      write.csv2(data.full, "./datasets/BMJV.csv", row.names = F)
      saveRDS(data.full, "./datasets/BMJV.rds")
