    ...
    #Save Link to "Referentenentwurf" / Link to "Regierungsentwurf" / Link to "Verkündetes Gesetz" / names of actor (who submitted a statement) / links to the statments
        ...
        #1."Referentenentwurf" keep only information of the last
          #Link
            Referentenentwurf.Link <- html_attr(
              ...
              Referentenentwurf.Link <- ifelse(length(Referentenentwurf.Link)>0,
                                               paste0("https://www.bmas.de",Referentenentwurf.Link), NA)
            
          #Date
            if(!is.na(Referentenentwurf.Link)) {
            Referentenentwurf.Date <- html_text(
                                        html_nodes(page, css = "#content > div:nth-child(2) > div > div.article-content-wrapper > div.article-text-wrapper > div > h2"))[str_detect(html_text(html_nodes(page, css = "#content > div:nth-child(2) > div > div.article-content-wrapper > div.article-text-wrapper > div > h2")) %>% trimws(which = c("both")), "Referentenentwurf")][1] %>% # get link when headline "Referentenentwurf" ist
                                          gsub("Referentenentwurf ", "",.) %>%
                                          gsub("\\(", "",.) %>% #delte Brackets
                                          gsub("\\)", "",.) %>% #delte Brackets
                                          as.Date(., format='%d.%m.%Y')
            } else {Referentenentwurf.Date <- NA}
            ...
            #Link
              Regierungsentwurf.Link <- html_attr(
                ...
                Regierungsentwurf.Link <- ifelse(length(Regierungsentwurf.Link)>0,
                                                 paste0("https://www.bmas.de",Regierungsentwurf.Link), NA)
            
            
            #Date
                if(!is.na(Regierungsentwurf.Link)) {
                  Regierungsentwurf.Date <- html_text(
                    html_nodes(page, css = "#content > div:nth-child(2) > div > div.article-content-wrapper > div.article-text-wrapper > div > h2"))[str_detect(html_text(html_nodes(page, css = "#content > div:nth-child(2) > div > div.article-content-wrapper > div.article-text-wrapper > div > h2")) %>% trimws(which = c("both")), "Regierungsentwurf")][1] %>% # get link when headline "Regierungsentwurf" ist
                    gsub("Regierungsentwurf ", "",.) %>%
                    gsub("\\(", "",.) %>% #delte Brackets
                    gsub("\\)", "",.) %>% #delte Brackets
                    as.Date(., format='%d.%m.%Y') %>% unlist()
                } else {Regierungsentwurf.Date <- NA}
        ...
        #3."Gesetz"
          ...
          #Date
            if(!is.na(Gesetz.Link)>0) {
              Gesetz.Date <- html_text(
                html_nodes(page, css = "#content > div:nth-child(2) > div > div.article-content-wrapper > div.article-text-wrapper > div > h2"))[str_detect(html_text(html_nodes(page, css = "#content > div:nth-child(2) > div > div.article-content-wrapper > div.article-text-wrapper > div > h2")) %>% trimws(which = c("both")), "Gesetz")] %>% # get link when headline "Regierungsentwurf" ist
                gsub("Gesetz ", "",.) %>%
                gsub("\\(", "",.) %>% #delte Brackets
                gsub("\\)", "",.) %>% #delte Brackets
                as.Date(., format='%d.%m.%Y')
            } else {Gesetz.Date <- as.character(NA)}   #HERE IS THE PROBLEM WITH Gesetz.Date became numeric (without as.character())
          
                
      #Several informations within legislative process    
        #4.  Link to "Verkündetes Gesetz"
                
          statement.data <- html_text(html_nodes(page, xpath = "/html/body/div[1]/div/div/div[3]/div/div/div[2]/div/div[2]/div/div[2]/div[1]/div/div/div[2]/ul/li")) %>%
            trimws(which = c("both")) %>%
            gsub(":", "",.) %>% # delete ":" at the end
            as.Date(., format='%d.%m.%Y')
          
          if(length(statement.data)==0 | is.na(statement.data[1])) statement.data <- NA
          
          
          
          if(!is.na(statement.data)) {
            actors <- html_text(html_nodes(page, xpath = "/html/body/div[1]/div/div/div[3]/div/div/div[2]/div/div[2]/div/div[2]/div[1]/div/div/div[2]/ul/li/a")) %>%
              trimws(which = c("both")) %>%
              gsub("Stellungnahme ", "",.) %>%
              gsub("des ", "",.) %>%
              gsub("der ", "",.)
          ...
          if(!is.na(statement.data)) {
            statement.links <- paste0("https://www.bmas.de/",
                                      html_attr(
                                        html_nodes(page, xpath = "/html/body/div[1]/div/div/div[3]/div/div/div[2]/div/div[2]/div/div[2]/div[1]/div/div/div[2]/ul/li/a")
                                      , "href")
            )
              if(length(statement.links)==0) statement.links <-NA
          }  
          
          if(is.na(statement.data)) {
            statement.links <- rep(NA, length(actors))
          }
          
          
          if(is.na(statement.data)) statement.data <- rep(NA, length(actors))
        ...
        data.actors <- data.frame(links = append(as.character(data.actors$links), rep(data$links[i], n.actors)),
                           ...
                           Vorgangstyp = append(data.actors$Vorgangstyp, rep(data$Vorgangstyp[i], n.actors)),
                             ...
                             Gesetz.Date = append(data.actors$Gesetz.Date, rep(Gesetz.Date, n.actors)),
                               ...
                               statement.data = append(data.actors$statement.data, statement.data),
      ...
      #clear working space
        rm(Referentenentwurf.Link, Referentenentwurf.Date, Regierungsentwurf.Link, Regierungsentwurf.Date,
           Gesetz.Link, Gesetz.Date, actors, statement.data, statement.links)
